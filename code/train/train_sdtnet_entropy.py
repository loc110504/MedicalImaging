...
def prediction_entropy(probs, eps=1e-8, normalize=True):
    """
    Compute mean predictive entropy for segmentation probabilities.

    Args:
        probs: Tensor of shape [B, C, H, W], after softmax.
        eps: numerical stability for log.
        normalize: if True, divide by log(C), so entropy is approximately in [0, 1].

    Returns:
        Scalar tensor: mean entropy over batch and spatial dimensions.
        Lower entropy = more confident teacher.
    """
    probs = probs.clamp_min(eps)
    entropy_map = -(probs * torch.log(probs)).sum(dim=1)  # [B, H, W]

    if normalize:
        num_classes = probs.shape[1]
        entropy_map = entropy_map / np.log(num_classes)
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled in enumerate(trainloader):
            ...
            # Entropy-based Dynamic Teacher Switching:
            # The teacher with lower predictive entropy is treated as more certain/reliable.
            # If Teacher 1 has higher entropy than Teacher 2, use Teacher 2, and vice versa.
            entropy_tea1 = prediction_entropy(outputs_soft_teacher1)
            entropy_tea2 = prediction_entropy(outputs_soft_teacher2)
            ...
            writer.add_scalar("train/entropy_teacher1", entropy_tea1.item(), iter_num)
            writer.add_scalar("train/entropy_teacher2", entropy_tea2.item(), iter_num)
            writer.add_scalar("train/selected_teacher", mode, iter_num)
