...
def get_threshold_curriculum(iter_num, train_args):
    """
    Confidence curriculum for agreement-only pseudo-labels.

    Early stage:
        high threshold -> few but very reliable pseudo-labels.

    Late stage:
        lower threshold -> more pseudo-label coverage.
    """
    if not train_args.use_threshold_curriculum:
        return train_args.pseudo_agree_thresh
...
def build_mt_confidence_pseudo_label(
    ...
    """
    Build reliable soft pseudo-labels through Mean Teacher agreement logic.
    ...
    soft_pseudo_label = mean_pseudo / (mean_pseudo.sum(dim=1, keepdim=True) + eps)

    reliable_mask = reliable_agree.float().unsqueeze(1)
    pseudo_conf = min_conf.unsqueeze(1)
...
def train(train_args, snapshot_path):
    ...
    for epoch_num in iterator:
        for sampled_batch in trainloader:
            ...
            # -------------------------
            cur_agree_thresh = get_threshold_curriculum(
                iter_num=iter_num,
            ...
            if iter_num % 200 == 0:
                logging.info(
                    ...
                    'agree_th=%.4f, reliable=%f, agree=%f, pseudo_conf=%f',
                    iter_num,
                    loss.item(),
                    loss_pce.item(),
                    loss_pseudo.item(),
