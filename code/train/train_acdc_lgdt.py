...
parser.add_argument("--exp", type=str, default="LED_SDT", help="experiment name")
...
def train(cfg, snapshot_path):
    ...
    db_val = ACDCDataSets(base_dir=cfg.root_path, fold=cfg.fold, split="val")
    ...
    for _ in iterator:
        for _, sampled in enumerate(trainloader):
            ...
            if cfg.debug_shapes and iter_num == 0:
                logging.info("Shapes logits_s=%s logits_l=%s logits_g=%s", tuple(logits_s.shape), tuple(logits_l.shape), tuple(logits_g.shape))
                logging.info("Shapes low_s=%s low_l=%s low_g=%s", tuple(out["low_s"].shape), tuple(out["low_l"].shape), tuple(out["low_g"].shape))
                logging.info("Shapes high_s=%s high_l=%s high_g=%s", tuple(out["high_s"].shape), tuple(out["high_l"].shape), tuple(out["high_g"].shape))

            loss_ce_s = ce_loss(logits_s, scrib)
            loss_dice_s = dice_loss(probs_s, scrib.unsqueeze(1))
            loss_scrib = loss_ce_s + loss_dice_s

            loss_aux = 0.5 * (ce_loss(logits_l, scrib) + ce_loss(logits_g, scrib))
            ...
            conf_mask = teacher_confidence_mask(selected_probs, threshold=cfg.teacher_conf_threshold)
            ...
            feat_low_t = select_teacher_feature(out["low_l"], out["low_g"], select_local=select_local).detach()
            feat_high_t = select_teacher_feature(out["high_l"], out["high_g"], select_local=select_local).detach()
            ...
            loss_consensus = masked_symmetric_kl_loss(probs_a=probs_l, probs_b=probs_g, mask=easy_mask)

            loss_div = logits_s.sum() * 0.0

            consistency_weight = get_current_consistency_weight(iter_num // 300, cfg)
            pseudo_weight = cfg.lambda_pseudo * consistency_weight
            hico_weight = cfg.lambda_hico * consistency_weight
            ...
            if iter_num % 400 == 0:
                logging.info("iteration %d : loss %.6f pseudo %.6f", iter_num, loss.item(), loss_pseudo.item())
            ...
            if iter_num > 1 and iter_num % 400 == 0:
                ...
                if performance > best_performance:
                    ...
                    save_best = os.path.join(snapshot_path, f"{cfg.model}_best_model.pth")
