...
def parse_args():
    ...
    parser.add_argument('--exp', type=str,
                        default='DMSPS_Stage1_v2', help='experiment_name')
...
def build_dual_feedback_soft_pseudo(
    outputs_soft1,
    outputs_soft2,
    label,
    agree_thresh=0.7,
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for iter, sampled_batch in enumerate(trainloader):
            ...
            pseudo_info = build_dual_feedback_soft_pseudo(
            ...
            loss_pse_sup_soft = 0.5 * (
                masked_soft_ce_loss(outputs, pseudo_info['soft_pseudo_label'], pseudo_info['reliable_mask']) +
                masked_soft_ce_loss(outputs_aux1, pseudo_info['soft_pseudo_label'], pseudo_info['reliable_mask'])
            )
            ...
            reliable_ratio_value = pseudo_info['reliable_ratio'].item()
            agreement_ratio_value = pseudo_info['agreement_ratio'].item()
            disagreement_ratio_value = pseudo_info['disagreement_ratio'].item()
            writer.add_scalar('info/reliable_ratio', reliable_ratio_value, iter_num)
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                logging.info(
                    'iteration %d : loss : %f, loss_ce: %f, loss_pse_sup_soft: %f, reliable: %f, agree: %f, disagree: %f' 
                    %(
                        iter_num,
                        loss.item(),
                        loss_ce.item(),
                        loss_pse_sup_soft.item(),
                        reliable_ratio_value,
