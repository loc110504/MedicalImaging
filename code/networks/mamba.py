...
def _selective_scan_fallback(
    ...
    This path is much slower than the fused `mamba_ssm` kernels, but it keeps
    the model runnable when the optional backend or `causal_conv1d` is absent.
    The current repo uses variable B/C with shape [B, K, N, L].
    """
    if z is not None:
        raise NotImplementedError("Fallback selective scan does not support z != None")

    if B.dim() != 4 or C.dim() != 4:
        raise ValueError(
            "Fallback selective scan expects B and C with shape [B, K, N, L], "
            "got B={} and C={}".format(tuple(B.shape), tuple(C.shape))
        )
    ...
    d_state = A.shape[1]
    if dim % num_groups != 0:
        raise ValueError("Channel dim {} is not divisible by num_groups {}".format(dim, num_groups))
    ...
    state = u.new_zeros((batch_size, dim, d_state))
    output = u.new_zeros((batch_size, dim, seq_len))
    A_expanded = A.unsqueeze(0)
    D_expanded = D.view(1, -1)

    for index in range(seq_len):
        delta_t = delta[:, :, index]
        u_t = u[:, :, index]
        B_t = B[:, :, :, index].repeat_interleave(group_width, dim=1)
        C_t = C[:, :, :, index].repeat_interleave(group_width, dim=1)

        deltaA_t = torch.exp(delta_t.unsqueeze(-1) * A_expanded)
        deltaBu_t = delta_t.unsqueeze(-1) * B_t * u_t.unsqueeze(-1)
        state = deltaA_t * state + deltaBu_t
        output[:, :, index] = torch.sum(state * C_t, dim=-1) + D_expanded * u_t
...
class SS2D(nn.Module):
    ...
    def __init__(
        ...
        if selective_scan_fn is not None:
            ...
            self.selective_scan = _selective_scan_fallback
            if not SS2D._fallback_warning_shown:
                warnings.warn(
                    "mamba_ssm selective_scan backend is unavailable ({}). "
                    "Falling back to a slower pure PyTorch implementation.".format(
                        repr(_SELECTIVE_SCAN_IMPORT_ERROR)
                    ),
                    RuntimeWarning,
                )
                SS2D._fallback_warning_shown = True
